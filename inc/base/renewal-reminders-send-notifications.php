    ...
    public function sprr_send_subscriber_notification_emaill()
    {
        ...
        foreach ($subscription_details as $subscription_detail) {
            ...
            if ($user && $options_en == 'on') {
                ...
                $user_preferred_language = "en"; //The language preferred by the user so we can fetch the proper subject and content translation
                //TODO: $user_preferred_language = language from user preferences
                ...
                $subject = stripslashes_deep(esc_attr(get_option('email_subject_' . $user_preferred_language))); //We fetch the option regarding the lang
                $headers = array('Content-Type: text/html; charset=UTF-8');
                // $headers = array('From:',$title); 

                $body = sprr_renewalremindersemail($subscriber_details['first_name'], $subscriber_details['last_name'], $next_payment_date, $user_preferred_language); //Construct the body regarding lang
